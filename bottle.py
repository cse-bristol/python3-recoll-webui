...
if py3k: # pragma: no cover
    ...
    def touni(x, enc='utf8', err='strict'):
        """ Convert anything to unicode """
        return x if isinstance(x, str) else unicode(str(x), enc, err)
...
class Bottle(object):
    ...
    def route(self, path=None, method='GET', callback=None, name=None,
        ...
        def decorator(callback):
            ...
            if isinstance(callback, (bytes, str)): callback = load(callback)
...
class BaseRequest(DictMixin):
    ...
    def cookies(self):
        ...
        cookies = http.cookies.BaseCookie(self.environ.get('HTTP_COOKIE',''))
    ...
    def fullpath(self):
        ...
        return urllib.parse.urljoin(self.script_name, self.path.lstrip('/'))
...
def run(app=None, server='wsgiref', host='127.0.0.1', port=8080,
    ...
    try:
        ...
        if isinstance(server, (bytes, str)):
            server = load(server)
